...
for (const file of [jsSource,cssSource,htmlPath]) if (!fs.existsSync(file)) throw new Error(`dish photo v3.3 dependency missing: ${file}`);
...
for (const obsolete of ['pfc-scan-v28.js','pfc-scan-v28.css','pfc-dish-photo-v29.js','pfc-dish-photo-v29.css','pfc-dish-photo-v29-bootstrap.js']) {
  if (html.includes(obsolete)) throw new Error(`obsolete scan asset still injected before v3.3: ${obsolete}`);
}
...
for (const marker of [
  ...
  'retryTransient:true',
  '料理写真'
]) if (!built.includes(marker)) throw new Error(`dish photo v3.3 marker missing: ${marker}`);
const test = spawnSync(process.execPath,[path.join(root,'scripts','test-dish-photo-v30.mjs'),jsOut],{stdio:'inherit'});
if (test.status !== 0) throw new Error('dish photo v3.3 tests failed');
console.log(`PFC dish photo v3.3 applied (${jsHash}/${cssHash}).`);
