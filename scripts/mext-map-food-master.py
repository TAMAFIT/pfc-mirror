...
def name_variants(item):
    raw=[item.get('name'),item.get('baseName'),*(item.get('aliases') or [])]
    out=[]
    for value in raw:
        n=norm(value)
        if len(n)<2 or n in {norm(x) for x in GENERIC}: continue
...
def name_score(item, official_name):
    ...
    for s in name_variants(item):
        if s == off: cand=(100.0,'exact')
            ...
            ratio=min(len(s),len(off))/max(len(s),len(off))
            cand=(78+12*ratio,'contains')
        else:
            cand=(70*dice(s,off),'bigrams')
    ...
    for cue in STATE_CUES:
        ...
        if q and q in app:
            ...
            elif cue not in {'焼'}: best=(best[0]-10,best[1]+'-state')
    return max(0.0,min(112.0,best[0])),best[1]


def category_score(item,item_no):
    category=item.get('category') or 'other'; group=str(item_no)[:2]
    allowed=CATEGORY_GROUPS.get(category)
    if not allowed: return 0.0, None
    return (20.0,True) if group in allowed else (-28.0,False)
...
def score_candidate(item,official):
    ns,reason=name_score(item,official.get('officialName'))
    cs,cat_ok=category_score(item,official.get('itemNo'))
    sim=nutrition_similarity(item,official)
    nutr=0.0 if sim is None else 72*sim
    if sim is not None and sim < 0.35: nutr-=18
    total=ns+cs+nutr
    return {'itemNo':official.get('itemNo'),'officialName':official.get('officialName'),'score':round(total,2),'nameScore':round(ns,2),'nameReason':reason,'categoryCompatible':cat_ok,'nutritionSimilarity':None if sim is None else round(sim,4),'official':{k:official.get(k) for k in ('p','f','c','kcal','a')}}
...
def classify_item(item,rows):
    ...
    if item.get('category') in MEXT_UNLIKELY: unlikely=True
    else: unlikely=False
    ...
    has_mass=item.get('per100g') is not None
    sim=top.get('nutritionSimilarity'); cat_ok=top.get('categoryCompatible')
...
def markdown(report):
    ...
    if high:
        ...
        for x in high[:120]:
            m=x['mapping']; t=m['candidates'][0]; parity='same' if m['nutritionInParity'] else 'sync '+','.join(m['syncNeededFields'])
            lines.append(f"| {x['name']} | {t['itemNo']} | {t['officialName']} | {t['score']} | {m['margin']} | {parity} |")
    ...
    if review:
        ...
        for x in review[:60]:
            m=x['mapping']; t=m['candidates'][0]
            lines.append(f"| {x['name']} | {t['itemNo']} {t['officialName']} | {t['score']} | {m['margin']} |")
    return '\n'.join(lines)+'\n'
...
def main():
    ...
    master=json.loads(Path(args.food_master).read_text(encoding='utf-8'))
    url,label=core.discover_workbook(); body,ctype=core.fetch_bytes(url)
    ...
    for item in master['items']:
        m=classify_item(item,rows); counts[m['status']]=counts.get(m['status'],0)+1
        mapped.append({'name':item['name'],'runtimeIndex':item['runtimeIndex'],'legacyIndex':item.get('legacyIndex'),'category':item.get('category'),'source':item.get('source'),'per100g':item.get('per100g'),'mapping':m})
    ...
    print('MEXT_MAP '+json.dumps(counts,ensure_ascii=False,sort_keys=True))
    return 0
