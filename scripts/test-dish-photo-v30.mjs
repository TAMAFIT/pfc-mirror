...
if (!file) throw new Error('dish photo v3.3 path required');
...
for (const forbidden of ['@zxing/browser','tesseract.js','Open Food Facts','label-ocr','scan-v28-barcode']) {
  if (source.includes(forbidden)) throw new Error(`removed scan feature leaked into v3.3: ${forbidden}`);
}
...
if (!api || api.version !== '3.3.0') throw new Error('v3.3 API missing');
if (api.model !== 'gemini-3.5-flash-lite' || api.thinkingLevel !== 'minimal' || api.nutritionFromAI !== false || !api.conservativeVisual) throw new Error('v3.3 model/latency invariants failed');
if (api.requestTimeoutMs !== 32000 || api.imageMaxSide !== 1024 || Math.abs(api.jpegQuality - 0.8) > 1e-9) throw new Error('v3.3 request/image optimization invariants failed');
if (!api.latencyOptimized || !api.retryTransient) throw new Error('v3.3 resilience markers failed');
if (api.aiAmountAutoApplied !== false || api.aiVariantFlagsTrusted !== false || !api.requiresUserAmount) throw new Error('v3.3 confirmation invariants failed');

const payload = api.buildRequestPayload('abc123');
if (payload.modelPreference !== 'gemini-3.5-flash-lite' || payload.imageBase64 !== 'abc123') throw new Error('v3.3 payload model/image failed');
if (payload.generationConfig?.thinkingConfig?.thinkingLevel !== 'minimal') throw new Error('v3.3 must force minimal thinking');
if (payload.generationConfig?.maxOutputTokens !== 1024) throw new Error('v3.3 output token cap missing');
if (!String(payload.contents?.[0]?.parts?.[0]?.text || '').includes('具が見えないおにぎりは必ず「おにぎり」')) throw new Error('v3.3 concise safety prompt missing');

const joined = api.extractAiText({candidates:[{content:{parts:[{text:'{"foods":'},{text:'[]}'}]}}]});
if (joined !== '{"foods":[]}') throw new Error('v3.3 must join text parts');
const retryable = api.classifyUpstreamText('GASエラー: AI API HTTP 503: unavailable');
if (!retryable || !retryable.retryable) throw new Error('v3.3 503 must be retryable');
const nonRetryable = api.classifyUpstreamText('GASエラー: AI API HTTP 400: invalid request');
if (!nonRetryable || nonRetryable.retryable) throw new Error('v3.3 400 must expose error without retry');
if (api.classifyUpstreamText('{"foods":[]}') !== null) throw new Error('v3.3 valid JSON must not be classified as upstream error');
...
console.log('Dish photo v3.3 low-latency/resilience regression tests passed.');
